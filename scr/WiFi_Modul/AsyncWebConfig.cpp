#if defined(ESP32) // || defined(__ESP32 Dev Module__)
/*
...
const char * inputtypes[] = {"text","password","number","date","time","range","check","radio","select","color","float","break"};
...
"<img src='logo.png' alt='wallterbox' class='center' style='width:290px;height:62px;'></br>\n"
"<div class='titel'>Konfiguration wallterbox MAC: %s</div>\n"
"<form method='post'>\n";
...
const char HTML_ENTRY_CHECKBOX2[] PROGMEM =
"  <div class='zeile'><b>%s</b><input type='checkbox' %s name='%s'></div>\n";
...
" </fieldset></div>\n";
const char HTML_ENTRY_BREAK[] PROGMEM =
" <br>\n";
const char HTML_ENTRY_LINE[] PROGMEM =
" <hr>\n";
...
void makeBreak(char * buf , DESCRIPTION descr) {
  sprintf(buf,HTML_ENTRY_BREAK,descr.label);
}

void makeLine(char * buf , DESCRIPTION descr) {
  sprintf(buf,HTML_ENTRY_LINE,descr.label);
}
  ...
  if (!exit) {
    ...
    for (uint8_t i = 0; i<_count; i++) {
      switch (_description[i].type) {
        ...
        case INPUTBREAK: makeBreak(_buf,_description[i]);
          break;  

        case INPUTLINE: makeLine(_buf,_description[i]);
          break;  
